...
const STABLE: EntityRegistryEntry[] = [
  ...
  {
    key: "tpv_sales",
    description: "Histórico de ventas TPV por tienda (todas las tiendas, incluida Calpe). Cada tienda mantiene su propio registro de ventas directas.",
    storageKeys: [
      "tcgacademy_tpv_calpe_sales",
      "tcgacademy_tpv_bejar_sales",
      "tcgacademy_tpv_madrid_sales",
      "tcgacademy_tpv_barcelona_sales",
    ],
    event: DataHubEvents.TPV_SALES_UPDATED,
    pii: true,
    retentionMonths: 72,
    adapter: "@/services/tpvStoreSalesService",
    maturity: "stable",
    category: "fiscal",
    criticalJson: true,
    dependsOn: ["products"],
    notes: "Las ventas TPV son venta directa: NO entran en el inbox de pedidos (/admin/pedidos). Calpe comparte stock+libro central con la web pero mantiene aquí su histórico para visibilidad por tienda. Béjar/Madrid/Barcelona tienen aquí su único registro de ventas (su libro fiscal y stock están en tpv_invoices/tpv_stock).",
  },
  {
    key: "tpv_stock",
    description: "Stock independiente por tienda standalone (Béjar/Madrid/Barcelona). Calpe NO está aquí: usa el stock central del catálogo web.",
    storageKeys: [
      "tcgacademy_tpv_bejar_stock",
      "tcgacademy_tpv_madrid_stock",
      "tcgacademy_tpv_barcelona_stock",
    ],
    event: DataHubEvents.TPV_STOCK_UPDATED,
    pii: false,
    retentionMonths: 0,
    adapter: "@/services/tpvStoreStockService",
    maturity: "stable",
    category: "catalogo",
    criticalJson: true,
    dependsOn: ["products"],
    notes: "Mapa {productId: stock} por tienda. Una venta en Béjar NO descuenta stock web ni de Madrid. Calpe usa LocalProduct.stock central — sus ventas TPV pasan por persistProductPatch igual que un pedido web.",
  },
  {
    key: "tpv_invoices",
    description: "Libro fiscal independiente por tienda standalone (Béjar/Madrid/Barcelona) con cadena hash propia. Calpe NO está aquí: sus facturas entran en el libro central de TCG Academy SL.",
    storageKeys: [
      "tcgacademy_tpv_bejar_invoices",
      "tcgacademy_tpv_bejar_inv_chain",
      "tcgacademy_tpv_bejar_inv_counter",
      "tcgacademy_tpv_madrid_invoices",
      "tcgacademy_tpv_madrid_inv_chain",
      "tcgacademy_tpv_madrid_inv_counter",
      "tcgacademy_tpv_barcelona_invoices",
      "tcgacademy_tpv_barcelona_inv_chain",
      "tcgacademy_tpv_barcelona_inv_counter",
    ],
    event: DataHubEvents.TPV_INVOICES_UPDATED,
    pii: true,
    retentionMonths: 72,
    adapter: "@/services/tpvStoreInvoiceService",
    maturity: "stable",
    category: "fiscal",
    criticalJson: true,
    dependsOn: ["tpv_sales"],
