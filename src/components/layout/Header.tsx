...
import { ShoppingCart, Heart, User, Search, Menu, X, Settings, Bell, Package, Clock, ShieldCheck, Store } from "lucide-react"
...
const TRUST_MESSAGES = [
  { icon: Package, text: "Envío gratis desde 149€" },
  { icon: Clock, text: "Enviamos en menos de 24 horas" },
  { icon: ShieldCheck, text: "Pago 100% seguro" },
  { icon: Store, text: "4 tiendas físicas en España" },
]
...
function MobileTrustBar() {
  ...
  return (
    <div className="bg-[#1a3a5c] text-white text-xs py-1.5 lg:hidden text-center px-4">
      <span className="inline-flex items-center gap-1.5 transition-opacity duration-300" style={{ opacity: fading ? 0 : 1 }}>
        <Icon size={12} /> {text}
      </span>
    </div>
  )
...
export function Header() {
  ...
  return (
    <header className="bg-white border-b border-gray-200 shadow-sm">
      {/* Topbar — desktop */}
      <div className="bg-[#1a3a5c] text-white text-xs py-1.5 hidden lg:block">
        <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-0 opacity-90">
            <span className="flex items-center gap-1.5"><Package size={12} /> Envío gratis desde 149€</span>
            <span className="mx-4 border-l border-white/25 h-3" />
            <span className="flex items-center gap-1.5"><Clock size={12} /> Enviamos en menos de 24 horas</span>
          </div>
          <div className="flex items-center gap-4 opacity-90">
            <span className="flex items-center gap-1.5"><ShieldCheck size={12} /> Pago 100% seguro</span>
            <span className="mx-3 border-l border-white/25 h-3" />
            <Link href="/tiendas" className="flex items-center gap-1.5 hover:opacity-100"><Store size={12} /> 4 Tiendas físicas</Link>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center gap-4">
        ...
        <div className="flex-1 max-w-xl hidden lg:block relative" ref={desktopSearchRef}>
          ...
          {desktopDropdownOpen && (
            <SearchDropdown query={debouncedDesktopQuery} onSelect={closeDesktopSearch} />
          )}
        </div>
        ...
        <div className="px-4 pb-3 lg:hidden border-t border-gray-100" ref={mobileSearchRef}>
          ...
          {mobileDropdownOpen && (
            <div className="mt-2 bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden">
              <SearchDropdown query={debouncedMobileQuery} onSelect={closeMobileSearch} />
            </div>
          )}
        </div>
      )}
