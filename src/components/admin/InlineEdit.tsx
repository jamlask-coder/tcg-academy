    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="ml-1 flex-shrink-0 rounded p-0.5 text-gray-400 opacity-0 transition-all group-hover:opacity-100 hover:bg-gray-100 hover:text-[#2563eb]"
          title="Editar"
        >
          <Pencil size={12} />
        </button>
    ...
    <span className={`inline-flex items-center gap-1 ${className}`}>
      {type === "textarea" ? (
        <textarea
          ...
          onKeyDown={handleKey}
          rows={3}
          className="w-full resize-none rounded-lg border-2 border-[#2563eb] px-2 py-1 text-sm focus:outline-none"
        />
      ) : (
        <input
          ...
          onChange={(e) => setDraft(e.target.value)}
          onKeyDown={handleKey}
          className="w-full rounded-lg border-2 border-[#2563eb] px-2 py-1 text-sm focus:outline-none"
        />
      )}
      <button
        onClick={handleSave}
