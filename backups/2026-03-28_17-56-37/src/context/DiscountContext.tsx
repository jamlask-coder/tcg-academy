...
interface DiscountContextValue {
  ...
  setDiscount: (productId: number, d: Partial<Omit<ProductDiscount, "productId">>) => void
  removeDiscount: (productId: number) => void
  setPriceOverride: (productId: number, prices: Partial<Omit<PriceOverride, "productId">>) => void
  bulkSetDiscount: (productIds: number[], pct: number, active: boolean, endsAt?: string) => void
  getEffectivePrice: (product: LocalProduct, role: "cliente" | "mayorista" | "tienda" | "admin" | null) => {
    displayPrice: number
    comparePrice?: number
    hasDiscount: boolean
    discountPct: number
...
export function DiscountProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      if (o) setPriceOverrides(JSON.parse(o))
    } catch { /* ignore */ }
  }, [])
  ...
  const setDiscount = useCallback((productId: number, d: Partial<Omit<ProductDiscount, "productId">>) => {
    setDiscounts((prev) => ({
      ...prev,
      [productId]: Object.assign({ pct: 0, active: false }, prev[productId], d, { productId }) as ProductDiscount,
    }))
  }, [])
  ...
  const setPriceOverride = useCallback((productId: number, prices: Partial<Omit<PriceOverride, "productId">>) => {
    setPriceOverrides((prev) => ({
      ...prev,
      [productId]: Object.assign({}, prev[productId], prices, { productId }) as PriceOverride,
    }))
  }, [])

  const bulkSetDiscount = useCallback((productIds: number[], pct: number, active: boolean, endsAt?: string) => {
    setDiscounts((prev) => {
      const next = { ...prev }
  ...
  const getEffectivePrice = useCallback(
    (product: LocalProduct, role: "cliente" | "mayorista" | "tienda" | "admin" | null) => {
      ...
      if (role === "mayorista") {
        ...
        priceLabel = "PVP Mayorista"
      } else if (role === "tienda") {
        displayPrice = baseStore
        priceLabel = "PVP Tienda"
      } else if (role === "admin") {
        displayPrice = basePrice
        priceLabel = "PVP General"
      } else {
        displayPrice = basePrice
      }
      ...
      const discountPct = hasDiscount ? Math.round((1 - displayPrice / compareRef!) * 100) : 0
  ...
  return (
    <DiscountContext.Provider value={{
      discounts, priceOverrides,
      setDiscount, removeDiscount, setPriceOverride, bulkSetDiscount,
      getEffectivePrice, saveToStorage,
    }}>
      {children}
