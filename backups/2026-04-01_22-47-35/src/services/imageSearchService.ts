...
async function searchPokemonSets(query: string): Promise<ImageSearchResult[]> {
  try {
    ...
    return data.data
      ...
      .map((s) => ({ url: s.images.logo, label: s.name, source: "Pokémon TCG" }));
  } catch {
    return [];
  }
}
...
async function searchPokemonCards(query: string): Promise<ImageSearchResult[]> {
  try {
    ...
    return data.data
      ...
      .map((c) => ({ url: c.images.large, label: c.name, source: "Pokémon TCG" }));
  } catch {
    return [];
  }
}

async function searchScryfallCards(query: string): Promise<ImageSearchResult[]> {
  try {
    const res = await fetch(
