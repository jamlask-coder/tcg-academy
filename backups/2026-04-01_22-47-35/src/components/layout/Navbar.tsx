...
const CM_SPRITE = "//static.cardmarket.com/img/af3476fd66bb1585352a2cfa4cd1ebf7/spriteSheets/ssGamesBig.png";
        ...
        <Container>
          <div className="flex items-center" style={{ minHeight: NAV_HEIGHT }}>
            ...
            <div className="flex flex-1 items-stretch min-w-0">
              ...
              {NAVBAR_GAMES.map(({ slug, label, href, color, abbrev, logoSrc }) => {
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      className="-mb-px flex flex-1 items-center justify-center border-b-2 transition-all duration-200 relative z-10"
                      style={{
                        borderBottomColor: active || open ? color : "transparent",
                        background: active || open ? `${color}12` : "transparent",
                      }}
                      title={label}
                    >
                      <GameLogo
              ...
              >
                <button
                  ...
                  className={`-mb-px flex items-center gap-1 border-b-2 px-3.5 text-sm font-semibold transition-all relative z-10 ${
                    activeItem === OTROS_KEY
            ...
            {/* ── Separator ────────────────────────────────────────────────── */}
            <div className="mx-2 h-5 w-px flex-shrink-0 bg-white/20 relative z-10" />

            {/* ── Eventos ──────────────────────────────────────────────────── */}
            <div onMouseEnter={() => setActiveItem(null)} className="flex items-stretch">
              <Link
                href="/eventos"
                className={`-mb-px flex items-center border-b-2 px-3.5 text-sm font-semibold whitespace-nowrap transition relative z-10 ${
            ...
            {/* ── Tiendas ──────────────────────────────────────────────────── */}
            <div onMouseEnter={() => openItem(TIENDAS_KEY)} className="flex items-stretch">
              <button
                aria-label="Ver nuestras tiendas"
                aria-expanded={activeItem === TIENDAS_KEY}
                className={`-mb-px flex items-center gap-1 border-b-2 px-3.5 text-sm font-semibold whitespace-nowrap transition relative z-10 ${
            ...
            {/* ── Profesionales ────────────────────────────────────────────── */}
            <div onMouseEnter={() => openItem(MAYORISTAS_KEY)} className="flex items-stretch">
              <button
                aria-label="Ver soluciones para profesionales"
                aria-expanded={activeItem === MAYORISTAS_KEY}
                className={`-mb-px flex items-center gap-1 border-b-2 px-3.5 text-sm font-semibold whitespace-nowrap transition relative z-10 ${
                  activeItem === MAYORISTAS_KEY || pathname.startsWith("/mayoristas")
                    ? "border-amber-400 text-amber-300"
                    : "border-transparent text-white/80 hover:text-white"
                }`}
              >
