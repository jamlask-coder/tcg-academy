            ...
            {results.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="whitespace-nowrap text-sm font-bold text-[#2563eb]">
                    {p.price.toFixed(2)}€
                  </p>
                </Link>
              );
      ...
      <form onSubmit={handleSubmit} className="flex items-stretch">
        ...
        >
          <input
            ...
            className={`h-full w-[90px] xl:w-[118px] px-3 text-xs text-white placeholder:text-white/50 focus:outline-none border-r transition-colors ${
              error ? "border-red-400 bg-red-900/30" : "bg-white/10 border-white/20 focus:bg-white/20"
            }`}
          />
          <input
            type="password"
          ...
          <button
            type="submit"
            disabled={loading}
            className="h-full px-4 text-xs font-semibold bg-amber-500 text-white hover:bg-amber-400 transition disabled:opacity-60 whitespace-nowrap border-r border-amber-600"
          >
            {loading ? "…" : "Entrar"}
          </button>
          <Link
            href="/registro"
            className="flex h-full items-center px-4 text-xs font-semibold bg-white/15 text-white hover:bg-white/25 transition whitespace-nowrap"
          >
            Registrarse
          </Link>
        </div>
...
export function Header() {
  ...
  return (
    <header className="border-b border-white/10" style={{ background: "linear-gradient(to right, #0a0f1a 0%, #1e3a8a 55%, #2563eb 100%)" }}>
      {/* Topbar — desktop */}
      <div
        ...
        >
          <form onSubmit={handleDesktopSubmit}>
            <input
              ...
              className="h-8 w-full rounded-xl border-2 border-white/25 bg-white/12 pr-8 pl-3 text-xs text-white placeholder:text-white/50 transition focus:border-white/60 focus:bg-white/20 focus:outline-none"
              autoComplete="off"
            />
            <button
              type="submit"
