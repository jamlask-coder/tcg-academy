          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
            ...
            <div className="divide-y divide-gray-100">
              {templates.map((t) => (
                ...
                >
                  <p
                    className={`truncate text-sm font-semibold ${selected.id === t.id ? "text-[#1a3a5c]" : "text-gray-800"}`}
                  >
                    {t.name}
                  </p>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white px-4 py-3">
            <div className="flex gap-1">
              {(
                ...
                <button
                  key={id}
                  onClick={() => setTab(id)}
                  className={`flex min-h-[36px] items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition ${tab === id ? "bg-[#1a3a5c] text-white" : "text-gray-600 hover:bg-gray-100"}`}
                >
                  <Icon size={14} /> {label}
                </button>
              ))}
            ...
            {tab !== "log" && (
              <button
                ...
                className="flex min-h-[36px] items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-gray-600 transition hover:border-[#1a3a5c] hover:text-[#1a3a5c]"
              >
            ...
            <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5">
              <div>
                ...
                <input
                  ...
                  className="h-10 w-full rounded-xl border-2 border-gray-200 px-3 text-sm transition focus:border-[#1a3a5c] focus:outline-none"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-gray-600">
                ...
                <textarea
                  ...
                  rows={20}
                  spellCheck={false}
                  className="w-full resize-y rounded-xl border-2 border-gray-200 px-3 py-2 font-mono text-xs transition focus:border-[#1a3a5c] focus:outline-none"
                />
              </div>
              <div className="flex gap-3">
                ...
                <button
                  onClick={saveChanges}
                  disabled={!editing}
                  className="flex min-h-[44px] items-center gap-2 rounded-xl bg-[#1a3a5c] px-5 py-2.5 text-sm font-bold text-white transition hover:bg-[#15304d] disabled:opacity-40"
                >
