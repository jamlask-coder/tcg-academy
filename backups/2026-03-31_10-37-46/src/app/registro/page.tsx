  ...
  const inputCls = (hasError: boolean) =>
    `h-11 w-full rounded-xl border-2 px-4 text-sm transition-all focus:outline-none focus:ring-2 ${
      hasError
        ...
        : "border-gray-200 focus:border-[#1a3a5c] focus:ring-[#1a3a5c]/10"
    }`;
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #0f2744 0%, #1a3a5c 55%, #1e4976 100%)",
        }}
      >
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          />
          <div
            className="absolute bottom-32 right-10 h-28 w-20 -rotate-6 rounded-xl bg-amber-400/10 shadow-2xl"
          ...
          <ul className="space-y-4">
            {FEATURES.map(({ icon: Icon, text }) => (
              <li key={text} className="flex items-center gap-3">
                ...
                <span className="text-base font-medium text-blue-100">{text}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center bg-white px-6 py-12">
        ...
        <Link href="/" className="mb-8 flex items-center gap-2 lg:hidden">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#1a3a5c]">
        ...
        >
          <div className="mb-8">
            ...
            <p className="mt-1 text-gray-500">Es gratis y solo tarda un minuto</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-3">
              <label className="flex cursor-pointer items-start gap-2.5 select-none">
                <input {...register("terminos")} type="checkbox" className="sr-only" />
                <div
                ...
                >
                  {watchedTerminos && <Check size={12} className="text-white" strokeWidth={3} />}
                </div>
                <span className="text-sm text-gray-600">
                  He leído y acepto los{" "}
                  <Link href="/terminos" target="_blank" className="font-medium text-[#1a3a5c] hover:underline">
                    términos y condiciones
                  </Link>{" "}
                  y la{" "}
                  <Link href="/privacidad" target="_blank" className="font-medium text-[#1a3a5c] hover:underline">
              ...
              {errors.terminos && (
                <p className="text-xs text-red-500">{errors.terminos.message}</p>
              )}
              ...
              <label className="flex cursor-pointer items-center gap-2.5 select-none">
                ...
                >
                  {watchedComunicaciones && <Check size={12} className="text-white" strokeWidth={3} />}
                </div>
                <span className="text-sm text-gray-600">
