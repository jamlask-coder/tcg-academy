...
export async function searchScryfallCards(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const data = await apiFetch<{ data: Record<string, any>[]; total_cards: number }>(
...
export async function getScryfallCard(id: string): Promise<ExternalCardData | null> {
  const cKey = `card_${id}`;
  const cached = readCache<ExternalCardData>(cKey);
  if (cached) return cached;
...
export async function getScryfallSets(): Promise<CardSet[]> {
  ...
  const result: CardSet[] = data.data
    ...
    .filter((s) =>
      ["expansion", "core", "masters", "commander", "draft_innovation"].includes(
        s.set_type as string,
      ),
    )
    .sort(
      (a, b) =>
