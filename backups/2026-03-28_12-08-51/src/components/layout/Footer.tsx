    ...
    <footer className="bg-[#1a3a5c] text-white">
      <div className="max-w-[1180px] mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          {/* Tiendas */}
          <div>
            <h3 className="font-bold text-xs uppercase tracking-wider mb-4 text-white/50">Tiendas Fisicas</h3>
            <ul className="space-y-2.5">
              {[
                ["Calpe, Alicante", "/tiendas/calpe"],
                ["Bejar, Salamanca", "/tiendas/bejar"],
          ...
          {/* Info + Newsletter */}
          <div>
            <h3 className="font-bold text-xs uppercase tracking-wider mb-4 text-white/50">Informacion</h3>
            <ul className="space-y-2.5 mb-6">
              {[
                ["Contacto", "/contacto"],
                ["Sobre nosotros", "/contacto"],
                ["Mayoristas B2B", "/mayoristas"],
                ["Envios y devoluciones", "/contacto"],
                ["Politica de privacidad", "/cuenta/datos"],
              ].map(([l, h]) => (
                <li key={h}><Link href={h} className="text-blue-200 text-sm hover:text-white transition">{l}</Link></li>
              ))}
            </ul>
            ...
            <form className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder:text-blue-300 focus:outline-none focus:border-white/50"
              />
              <button
                type="submit"
                className="bg-white text-[#1a3a5c] font-semibold text-sm px-4 py-2 rounded-lg hover:bg-blue-50 transition">
                OK
              </button>
            </form>
          </div>
