  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Actualiza tu perfil y contraseña</p>
      </div>
            ...
            <div key={key}>
              ...
              <input
                ...
                className={`w-full h-11 px-4 border-2 border-gray-200 rounded-xl text-sm focus:outline-none transition ${
                  key === "email" ? "bg-gray-50 text-gray-400 cursor-not-allowed" : "focus:border-[#1a3a5c]"
                }`}
              />
              {key === "email" && (
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
          ...
          {(["current", "next", "confirm"] as const).map((key) => (
            <div key={key}>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                {key === "current" ? "Contraseña actual" : key === "next" ? "Nueva contraseña" : "Confirmar nueva contraseña"}
              </label>
        ...
        <div className="bg-blue-50 border border-blue-100 text-blue-600 text-xs rounded-xl px-4 py-3">
          El cambio de contraseña estara disponible cuando se conecte con el backend de autenticacion.
        </div>
      </div>
    </div>
  )
