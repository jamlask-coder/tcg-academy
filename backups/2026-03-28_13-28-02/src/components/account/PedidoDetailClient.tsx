...
import { ChevronLeft, Download, Package, MapPin, CreditCard, Truck, Copy, Check, RefreshCw } from "lucide-react"
...
export function PedidoDetailClient({ id }: Props) {
  ...
  const handleDownloadInvoice = () => {
    ...
    const data = buildInvoiceFromOrder({
      ...
      items: order?.items?.map(i => ({ name: i.name, quantity: i.qty ?? 1, price: i.price })) ?? [],
      total: order?.total ?? 0,
    }, invNum)
    printInvoice(data)
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-8 flex-wrap">
        <Link href="/cuenta/pedidos" className="text-gray-500 hover:text-gray-700 min-w-[44px] min-h-[44px] flex items-center justify-center">
          <ChevronLeft size={20} />
        </Link>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex-1 flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
              <span className="font-mono font-bold text-gray-800 text-sm tracking-widest">{trackingNumber}</span>
              <button
            ...
            <a
              href={`https://www.correos.es/ss/Satellite/site/app-buscador-de-envios/sidecar-buscador-de-envios?s=${trackingNumber}`}
              target="_blank"
              rel="noreferrer"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6">
        ...
        <div className="space-y-4">
          {[
            { name: "Naruto Mythos: Konoha Shido Booster Box (24 sobres)", qty: 1, price: 79.95 },
            ...
            <div key={i} className="flex items-center justify-between gap-4 py-3 border-b border-gray-100 last:border-0">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-orange-50 rounded-xl flex items-center justify-center flex-shrink-0 text-xl">🍃</div>
                <div>
                  <p className="text-sm font-medium text-gray-800">{item.name}</p>
                  <p className="text-xs text-gray-500">Cantidad: {item.qty}</p>
                </div>
              </div>
              <p className="font-bold text-gray-900 whitespace-nowrap">{(item.price * item.qty).toFixed(2)}€</p>
            </div>
          ))}
          <div className="pt-2 space-y-2 text-sm">
            <div className="flex justify-between text-gray-600"><span>Subtotal</span><span>109.85€</span></div>
            <div className="flex justify-between text-gray-600"><span>Envío</span><span className="text-green-600">Gratis</span></div>
            <div className="flex justify-between font-bold text-gray-900 text-base pt-2 border-t border-gray-100">
              <span>Total</span><span>109.85€</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            ...
            <p className="text-xs text-gray-400 mt-2">Pago procesado correctamente</p>
          </div>
        </div>
      </div>
    </div>
