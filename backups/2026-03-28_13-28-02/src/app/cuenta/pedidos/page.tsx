...
const MOCK_ORDER_IDS = new Set(["TCG-20250128-001", "TCG-20250115-002", "TCG-20241230-003", "TCG-20241201-004"])
...
const MOCK_ORDERS: Order[] = [
  ...
  },
  {
    ...
    status: "Entregado",
    total: 249.95,
    items: [{ name: "Bloomburrow Collector Booster Display", qty: 1, price: 249.95 }],
  },
  {
    id: "TCG-20241230-003",
    ...
    status: "Entregado",
    total: 74.85,
    items: [{ name: "One Piece OP-08 Two Legends Booster Box", qty: 1, price: 74.95 }],
  },
  {
    id: "TCG-20241201-004",
    ...
    status: "Cancelado",
    total: 19.95,
    items: [{ name: "Yu-Gi-Oh! Tin of Ancient Battles 2024", qty: 1, price: 19.95 }],
  },
]
...
function normalizeStatus(s: string): OrderStatus {
  const map: Record<string, OrderStatus> = {
    procesando: "Procesando", pendiente: "Pendiente",
    enviado: "Enviado", entregado: "Entregado", cancelado: "Cancelado",
  }
  return map[s.toLowerCase()] ?? "Procesando"
}
                      ...
                      {order.items.map((item, i) => {
                        ...
                        return (
                          <p key={i} className="text-sm text-gray-600">
                            {qty}× {item.name} — <span className="font-medium">{(item.price * qty).toFixed(2)}€</span>
                          </p>
                        )
                      })}
                    </div>
