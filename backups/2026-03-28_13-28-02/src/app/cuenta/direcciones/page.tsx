...
export default function DireccionesPage() {
  ...
  const [form, setForm] = useState<Partial<Address>>({
    label: "Casa", nombre: "", apellidos: "", calle: "",
    numero: "", piso: "", cp: "", ciudad: "", provincia: "", pais: "ES",
  })
  ...
  const handleSave = () => {
    ...
    setShowForm(false)
    setEditId(null)
    setForm({ label: "Casa", nombre: "", apellidos: "", calle: "", numero: "", piso: "", cp: "", ciudad: "", provincia: "", pais: "ES" })
  }

  const handleDelete = (id: string) => setAddresses((prev) => prev.filter((a) => a.id !== id))
  const handleSetDefault = (id: string) =>
    setAddresses((prev) => prev.map((a) => ({ ...a, predeterminada: a.id === id })))
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Gestiona tus direcciones de envio</p>
        </div>
        <button
      ...
      {addresses.length === 0 && !showForm && (
        <div className="bg-white border border-gray-200 rounded-2xl p-16 text-center">
          ...
          <p className="font-bold text-gray-700 mb-2">No tienes direcciones guardadas</p>
          <button
          ...
          >
            <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {addr.predeterminada && (
                    <span className="flex items-center gap-1 text-[10px] font-bold text-[#1a3a5c] bg-blue-50 px-2 py-0.5 rounded-full">
                      <Star size={10} className="fill-[#1a3a5c]" /> Predeterminada
                    </span>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-2 flex-shrink-0">
                <button onClick={() => handleEdit(addr)} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-[#1a3a5c] transition">
                  <Pencil size={13} /> Editar
                </button>
                {!addr.predeterminada && (
                  <button onClick={() => handleSetDefault(addr.id)} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-[#1a3a5c] transition">
                    <Star size={13} /> Predeterminar
                  </button>
                )}
                <button onClick={() => handleDelete(addr.id)} className="flex items-center gap-1.5 text-xs text-red-400 hover:text-red-600 transition">
      ...
      {showForm && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <h2 className="font-bold text-gray-900 mb-4">{editId ? "Editar direccion" : "Nueva direccion"}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Etiqueta</label>
              <select value={form.label} onChange={set("label")} className={inputCls}>
            ...
            <div>
              ...
              <input type="text" value={form.calle ?? ""} onChange={set("calle")} className={inputCls} />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Numero</label>
                <input type="text" value={form.numero ?? ""} onChange={set("numero")} className={inputCls} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Piso</label>
                <input type="text" value={form.piso ?? ""} onChange={set("piso")} className={inputCls} />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Codigo postal</label>
              <input type="text" value={form.cp ?? ""} onChange={set("cp")} className={inputCls} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Ciudad</label>
              <input type="text" value={form.ciudad ?? ""} onChange={set("ciudad")} className={inputCls} />
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 mb-1">Pais</label>
              <select value={form.pais ?? "ES"} onChange={set("pais")} className={inputCls}>
                <option value="ES">España</option><option value="PT">Portugal</option>
                <option value="FR">Francia</option><option value="DE">Alemania</option>
              </select>
            </div>
          </div>
          <div className="flex gap-3 mt-6">
            <button onClick={handleSave} className="bg-[#1a3a5c] text-white font-bold px-6 py-2.5 rounded-xl text-sm hover:bg-[#15304d] transition">
              Guardar
            </button>
            <button onClick={() => { setShowForm(false); setEditId(null) }} className="border-2 border-gray-200 text-gray-600 font-semibold px-6 py-2.5 rounded-xl text-sm hover:border-gray-300 transition">
              Cancelar
            </button>
          </div>
        </div>
