...
interface ProductFormProps {
  ...
  onSubmitAndNew?: (data: ProductFormValues, tags: string[], images: string[]) => void
}
...
export function ProductForm({
  title, subtitle, defaultValues, initialTags = [], originalName,
  resetCategoryOnGame, showImageUpload, submitLabel, onSubmit, onSubmitAndNew,
}: ProductFormProps) {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<ProductFormValues>({
    ...
    defaultValues: { inStock: true, isNew: false, language: "", ...defaultValues },
  })
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          {subtitle && <p className="text-gray-500 text-sm mt-0.5 truncate max-w-md">{subtitle}</p>}
        </div>
      </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-5">
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" min="0" {...register("price", { valueAsNumber: true })} className={inputCls} placeholder="0.00" />
                  {errors.price && <p className={errorCls}>{errors.price.message}</p>}
                </div>
                <div>
                  <label className={labelCls}>PVP Mayorista (€) *</label>
                  <input type="number" step="0.01" min="0" {...register("wholesalePrice", { valueAsNumber: true })} className={inputCls} placeholder="0.00" />
                  {errors.wholesalePrice && <p className={errorCls}>{errors.wholesalePrice.message}</p>}
                </div>
                <div>
                  <label className={labelCls}>PVP Tienda TCG (€) *</label>
                  <input type="number" step="0.01" min="0" {...register("storePrice", { valueAsNumber: true })} className={inputCls} placeholder="0.00" />
                  {errors.storePrice && <p className={errorCls}>{errors.storePrice.message}</p>}
                </div>
                <div>
                  <label className={labelCls}>Precio anterior (€, tachado)</label>
                  <input type="number" step="0.01" min="0" {...register("comparePrice", { valueAsNumber: true })} className={inputCls} placeholder="0.00 (opcional)" />
                </div>
              </div>
            </section>
            ...
            <div className="flex flex-wrap gap-3 pb-6">
              ...
              {onSubmitAndNew && (
                <button type="button" onClick={handleSubmit(handleSaveAndNew)}
