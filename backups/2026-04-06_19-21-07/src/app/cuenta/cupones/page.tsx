    ...
    >
      ...
      <div
        className={`absolute top-0 bottom-0 left-0 w-1.5 ${
          isActive ? "bg-[#2563eb]" : isUsed ? "bg-gray-400" : "bg-gray-300"
        }`}
      />
      ...
      <div className="py-5 pr-5 pl-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          {/* Discount amount */}
          <div
            className={`flex items-center gap-2 ${isActive ? "text-[#2563eb]" : "text-gray-500"}`}
          >
            <div
            ...
            <div>
              <p
                className={`text-2xl leading-none font-black ${isActive ? "text-[#2563eb]" : "text-gray-400"}`}
              >
                {coupon.discountType === "percent"
                  ? `${coupon.value}%`
                  : `${coupon.value}€`}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div
            className={`flex items-center gap-2 rounded-lg border px-3 py-1.5 ${
              isActive
                ? "border-dashed border-[#2563eb] bg-blue-50"
                : "border-gray-200 bg-gray-100"
            }`}
          >
            <span
              className={`font-mono text-sm font-bold tracking-wider ${isActive ? "text-[#2563eb]" : "text-gray-400"}`}
            >
              {coupon.code}
            </span>
            {isActive && (
              <button
                ...
                className="ml-1 text-xs text-[#2563eb] hover:underline"
              >
                Copiar
              </button>
            )}
    ...
    <div>
      <div className="mb-6">
        <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
          <Gift size={22} className="text-[#2563eb]" /> Cupones y Descuentos
        </h1>
      ...
      {/* Redeem section */}
      <div className="mb-8 rounded-2xl bg-gradient-to-br from-[#2563eb] to-[#3b82f6] p-6 text-white">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="h-12 rounded-xl bg-white px-6 text-sm font-bold text-[#2563eb] transition hover:bg-blue-50 disabled:opacity-50"
          >
            Canjear
          </button>
        </div>
