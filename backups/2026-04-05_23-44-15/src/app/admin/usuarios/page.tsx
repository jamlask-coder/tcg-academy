...
export default function AdminUsuariosPage() {
  ...
  const confirmRoleChange = () => {
    ...
    setUsers((prev) =>
      prev.map((u) =>
        u.id === pending.userId ? { ...u, role: pending.newRole } : u,
      ),
    );
    if (selected?.id === pending.userId)
      setSelected((p) => (p ? { ...p, role: pending.newRole } : null));
    showToast(
      `Rol de ${pending.userName} actualizado a ${ROLE_LABELS[pending.newRole]}`,
    );
    setPending(null);
  };
        ...
        >
          ...
          >
            ...
            <div className="mb-5 flex items-center gap-3 rounded-xl bg-gray-50 p-3">
              <span
                className={`rounded-full px-2.5 py-1 text-xs font-bold capitalize ${ROLE_COLORS[pending.currentRole]}`}
              >
                {ROLE_LABELS[pending.currentRole]}
              </span>
              <ChevronRight size={14} className="flex-shrink-0 text-gray-400" />
              <span
                className={`rounded-full px-2.5 py-1 text-xs font-bold capitalize ${ROLE_COLORS[pending.newRole]}`}
              >
          ...
          {[...users]
            ...
            .map((u) => (
              ...
              >
                ...
                <span
                  className={`hidden flex-shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold sm:inline-block ${ROLE_COLORS[u.role]}`}
                >
                  {ROLE_LABELS[u.role]}
                </span>
