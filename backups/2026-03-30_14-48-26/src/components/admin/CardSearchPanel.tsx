...
import { getCardServiceForGame, isCardSearchSupported } from "@/services/cardImageService";
...
const GAME_LABELS: Record<string, { label: string; emoji: string; color: string; border: string; bg: string; text: string; btn: string }> = {
  pokemon: {
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        {game !== "yugioh" && (
          <div className="relative">
            ...
            {setsOpen && (
              <div className={`absolute top-full right-0 z-50 mt-1 max-h-48 w-64 overflow-y-auto rounded-xl border ${theme.border} bg-white shadow-xl`}>
                <button
                  type="button"
            ...
            {results.map((card) => (
              ...
              >
                {card.imageUrl ? (
                  ...
                  <div className={`flex aspect-[2/3] w-full items-center justify-center rounded-lg ${theme.bg} text-2xl`}>
                    {theme.emoji}
                  </div>
                )}
                ...
                {card.rarity && (
                  <span className={`text-[9px] ${theme.text}`}>{card.rarity}</span>
                )}
              </button>
            ))}
          </div>
