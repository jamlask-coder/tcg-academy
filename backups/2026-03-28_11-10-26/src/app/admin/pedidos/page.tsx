...
import { Search, ChevronDown, X, Check, Truck, Send } from "lucide-react"
import { ALL_ORDERS, type Order, type OrderStatus } from "@/data/mockData"
...
export default function AdminPedidosPage() {
  ...
  const updateStatus = (orderId: string, newStatus: OrderStatus, tracking?: string) => {
    setOrders((prev) =>
      prev.map((o) =>
        o.id === orderId
          ? { ...o, status: newStatus, ...(tracking ? { trackingNumber: tracking } : {}) }
          : o
      )
    )
    showToast(`Pedido ${orderId} actualizado a "${newStatus}". Email enviado al cliente.`)
  }
                    ...
                    <tr key={`${order.id}-expanded`}>
                      <td colSpan={6} className="px-4 pb-4 bg-blue-50/40">
                        <div className="pt-3 space-y-3">
                          ...
                          {/* Tracking input for "enviado" transition */}
