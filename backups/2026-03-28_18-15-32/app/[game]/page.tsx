...
const UPCOMING: Record<string, { name: string; date: string }[]> = {
  magic:       [{ name: "Final Fantasy — Set completo", date: "Junio 2025" }, { name: "Edge of Eternities", date: "Agosto 2025" }],
  pokemon:     [{ name: "Terastal Festival EX", date: "Mayo 2025" }, { name: "Stellar Crown 2", date: "Septiembre 2025" }],
...
export default async function GamePage({
  ...
  return (
    <div>
      {/* Hero */}
      <GameHero game={game} config={{ name, color, bgColor, description, emoji }} featuredProducts={bestsellers} />
      ...
      {bestsellers.length > 0 && (
        <section className="max-w-[1180px] mx-auto px-6 py-10">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Lo más vendido</h2>
              <p className="text-sm text-gray-500 mt-0.5">Los favoritos de la comunidad</p>
            </div>
            <Link href={`/${game}`} className="text-sm font-semibold flex items-center gap-1 hover:underline" style={{ color }}>
        ...
        <section className="max-w-[1180px] mx-auto px-6 pb-10">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Recién llegados a la tienda</p>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {newProducts.map((p) => (
        ...
        <section className="max-w-[1180px] mx-auto px-6 pb-10">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {upcoming.map(({ name: relName, date }) => (
              ...
              >
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0" style={{ backgroundColor: `${color}15` }}>
                  {emoji}
                </div>
                <div>
                  <p className="font-bold text-gray-900 text-sm leading-tight">{relName}</p>
                  <p className="text-xs font-semibold mt-1" style={{ color }}>{date}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="max-w-[1180px] mx-auto px-6 pb-16">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">
            Catálogo completo{" "}
            <span className="text-sm font-normal text-gray-400">({all.length} productos)</span>
          </h2>
        </div>
        {all.length === 0 ? (
          <div className="text-center py-24 border-2 border-dashed border-gray-200 rounded-2xl">
            <p className="text-gray-400 font-medium">Catálogo en construcción</p>
            <p className="text-sm text-gray-300 mt-1">Pronto tendrás más productos disponibles</p>
          </div>
        ) : (
