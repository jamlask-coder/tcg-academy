    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Saldo disponible:{" "}
          <strong className="text-[#1a3a5c] text-base">{balance} puntos</strong>
        </p>
          ...
          {POINTS_REDEMPTION_TABLE.map((option) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-lg font-black ${canRedeem ? "text-[#1a3a5c]" : "text-gray-300"}`}
                >
                  = {option.euros.toFixed(2)}€
                </span>
              </button>
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <div className="flex items-end gap-2">
              ...
              <span className="text-amber-200 text-lg font-bold mb-1">
                puntos
              </span>
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50">
                <th className="text-left px-4 py-2.5 font-semibold text-gray-600 rounded-l-xl">
                  Puntos necesarios
                </th>
                <th className="text-right px-4 py-2.5 font-semibold text-gray-600 rounded-r-xl">
                  Descuento obtenido
                </th>
              </tr>
            </thead>
            <tbody>
              {POINTS_REDEMPTION_TABLE.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-bold text-[#1a3a5c] text-base">
                    {row.euros.toFixed(2)}€
                  </td>
                </tr>
              ))}
            ...
            return (
              <div key={tx.id} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="text-right flex-shrink-0">
                  <p
                    className={`font-bold text-sm ${tx.points > 0 ? "text-green-600" : "text-red-500"}`}
                  >
                    {tx.points > 0 ? "+" : ""}
                    {tx.points} pts
                  </p>
