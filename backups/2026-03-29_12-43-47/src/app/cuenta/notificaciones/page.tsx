            ...
            return (
              <div
                ...
                className={`bg-white border rounded-2xl p-4 transition flex gap-4 ${
                  !notif.read
                    ? "border-[#1a3a5c]/20 shadow-sm"
                    : "border-gray-200"
                }`}
              >
                {/* Icon */}
                <div
                  className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${config.bg}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <p
                      className={`text-sm font-bold leading-snug ${notif.read ? "text-gray-700" : "text-gray-900"}`}
                    >
                      {notif.title}
                      {!notif.read && (
