...
function GameLogo({ game, name, color, emoji, mobile = false }: { game: string; name: string; color: string; emoji: string; mobile?: boolean }) {
  ...
  return (
    ...
    <img
      src={src}
      alt={name}
      className={`object-contain drop-shadow-lg ${mobile ? "h-14 max-w-[220px]" : "h-20"}`}
      style={{ width: "auto", maxWidth: "none", filter: `drop-shadow(0 0 20px ${color}60)` }}
      onError={() => {
        if (src.endsWith(".svg")) {
...
export function GameHero({ game, config }: Props) {
  ...
  return (
    <section
      ...
      style={{ background: `linear-gradient(135deg,#0a0f1a 0%,${color}22 45%,${color}35 100%)`, minHeight: 320 }}
    ...
    >
      ...
      <Container className="relative hidden lg:block">
        <div
          ...
          style={{ gridTemplateColumns: "320px 1fr 220px", gap: "1.5rem", minHeight: 310, paddingTop: "0.75rem", paddingBottom: "0.75rem" }}
        >
          {/* LEFT: Logo */}
          <motion.div
          ...
          >
            ...
            <motion.p
              ...
              className="text-center text-xs leading-relaxed text-white/55 max-w-[280px]"
            >
              {description}
            </motion.p>
          </motion.div>
          ...
          <div className="flex items-center justify-center">
            <div style={{ position: "relative", width: 460, height: 310, flexShrink: 0 }}>
              {cards.slice(0, 5).map((card, i) => {
                const f = FAN_5[i];
                return (
                  <HoloCard key={i} card={card} color={color} emoji={emoji} width={179} height={250}
