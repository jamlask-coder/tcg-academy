...
import { Receipt, Download, Search, ChevronDown, X, FileSpreadsheet, Calendar } from "lucide-react"
...
function StatusBadge({ status }: { status: Invoice["status"] }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${
      status === "pagada" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
    }`}>
    ...
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-sm transition">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 p-5 border-b border-gray-100">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className="text-xl font-bold text-[#1a3a5c]">{grandTotal.toFixed(2)}€</span>
          <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left border-b border-gray-100">
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-center w-12">Cant.</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600">Descripción</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-right hidden sm:table-cell">P.U. s/IVA</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-right hidden sm:table-cell">Total s/IVA</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-center hidden md:table-cell">IVA %</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-right hidden md:table-cell">IVA €</th>
              <th className="px-4 py-2.5 font-semibold text-gray-600 text-right">Total c/IVA</th>
            </tr>
          </thead>
          <tbody>
            {inv.items.map((item, i) => {
              ...
              return (
                <tr key={i} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-4 py-2.5 text-right text-gray-500 hidden sm:table-cell">{v.unitPriceNet.toFixed(2)}€</td>
                  <td className="px-4 py-2.5 text-right text-gray-500 hidden sm:table-cell">{v.totalNet.toFixed(2)}€</td>
                  <td className="px-4 py-2.5 text-center text-gray-500 hidden md:table-cell">{v.vatRate}%</td>
                  <td className="px-4 py-2.5 text-right text-gray-500 hidden md:table-cell">{v.vatAmount.toFixed(2)}€</td>
                  <td className="px-4 py-2.5 text-right font-semibold text-gray-900">{v.totalGross.toFixed(2)}€</td>
                </tr>
              )
            })}
          </tbody>
          <tfoot className="border-t-2 border-gray-200 bg-gray-50">
            <tr className="text-xs text-gray-500">
              <td colSpan={4} className="px-4 py-2.5 text-right hidden sm:table-cell">Base imponible:</td>
              <td colSpan={2} className="px-4 py-2.5 text-right hidden sm:table-cell font-medium text-gray-700">{baseTotal.toFixed(2)}€</td>
              <td className="sm:hidden" />
            </tr>
            <tr className="text-xs text-gray-500">
              <td colSpan={4} className="px-4 py-1.5 text-right hidden sm:table-cell">IVA total:</td>
              <td colSpan={2} className="px-4 py-1.5 text-right hidden sm:table-cell font-medium text-gray-700">{vatTotal.toFixed(2)}€</td>
              <td className="sm:hidden" />
            </tr>
            <tr>
              <td colSpan={5} className="px-4 py-2.5 text-right font-bold text-gray-900 hidden sm:table-cell">TOTAL FACTURA:</td>
              <td colSpan={2} className="px-4 py-2.5 text-right font-bold text-[#1a3a5c] text-base">{grandTotal.toFixed(2)}€</td>
            </tr>
          </tfoot>
        </table>
      </div>
...
export default function FacturasPage() {
  ...
  const isB2B = user?.role === "mayorista" || user?.role === "tienda" || user?.role === "admin"
    ...
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Descarga tus facturas con desglose de IVA</p>
        </div>
        {isB2B && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            />
            <button
              onClick={() => downloadPurchaseReport(filtered, reportPeriod || "completo")}
