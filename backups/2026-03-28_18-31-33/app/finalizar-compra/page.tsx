...
import { Shield, Truck, CheckCircle, CreditCard, ArrowLeft, Tag, Star, Store } from "lucide-react"
...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({
    ...
    envio: "estandar", pago: "tarjeta", tiendaRecogida: "",
  })
        ...
        <div className="lg:col-span-2">
          ...
          {step === "envio" && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
              ...
              {[
                { id: "estandar", label: "Envío estándar con GLS", sub: "Entrega en menos de 24h", price: total >= 149 ? "Gratis" : "3,99€" },
                ...
                <label key={opt.id} className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition ${form.envio === opt.id ? "border-[#1a3a5c] bg-blue-50/40" : "border-gray-200 hover:border-gray-300"}`}>
                  <input type="radio" name="envio" value={opt.id} checked={form.envio === opt.id}
                  ...
                  <div className="flex-1">
                    <div className="font-semibold text-gray-900 text-sm flex items-center gap-2">
                      {opt.id === "tienda" && <Store size={14} className="text-[#1a3a5c]" />}
                      {opt.label}
                    </div>
              ...
              {isStorePickup && (
                <div className="border-2 border-[#1a3a5c]/20 rounded-xl p-4 bg-blue-50/30 space-y-2">
                  <p className="text-sm font-semibold text-gray-800 mb-3">Selecciona la tienda de recogida:</p>
                  {TIENDAS.map(t => (
                    <label key={t.id} className={`flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition ${form.tiendaRecogida === t.id ? "border-[#1a3a5c] bg-white" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                      <input type="radio" name="tiendaRecogida" value={t.id}
          ...
          {step === "pago" && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-4">
              ...
              {[
                ...(isStorePickup
                  ? [{ id: "tienda", label: "Pago en tienda", sub: "Paga al recoger tu pedido en la tienda" }]
                  : []),
                ...(!isStorePickup
                  ? [
