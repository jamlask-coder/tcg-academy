...
function SavedToast() {
  return (
    <div className="fixed bottom-6 right-6 z-50 bg-[#1a3a5c] text-white px-4 py-2.5 rounded-2xl shadow-xl flex items-center gap-2 text-sm font-medium animate-fade-in">
      <Check size={14} className="text-green-300" /> Cambio guardado
    </div>
  )
}

export function InlineEdit({ value, type = "text", onSave, className = "", children, step, min }: InlineEditProps) {
  const { user } = useAuth()
  ...
  if (!editing) {
    return (
      <span className={`relative group inline-flex items-start gap-1 ${className}`}>
        {children ?? value}
        <button
  ...
  return (
    <span className={`inline-flex items-center gap-1 ${className}`}>
      ...
      <button onClick={handleSave} className="p-1 rounded-lg bg-green-500 text-white hover:bg-green-600 transition flex-shrink-0" title="Guardar">
        <Save size={13} />
      </button>
      <button onClick={handleCancel} className="p-1 rounded-lg bg-gray-200 text-gray-600 hover:bg-gray-300 transition flex-shrink-0" title="Cancelar">
        <X size={13} />
      </button>
      {showToast && <SavedToast />}
    </span>
