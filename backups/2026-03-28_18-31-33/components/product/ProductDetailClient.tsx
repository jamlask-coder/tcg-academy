...
function PriceDisplay({ product, color, inlinePrice, onPriceChange }: { product: LocalProduct; color: string; inlinePrice: number; onPriceChange: (v: string) => void }) {
  const { role } = useAuth()
      ...
      <div className="space-y-2">
        <div className="flex items-end gap-3">
          <span className="text-4xl font-bold" style={{ color }}>
            <InlineEdit type="number" step="0.01" min="0" value={inlinePrice} onSave={onPriceChange} className="inline-flex">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="bg-gray-100 text-gray-600 px-2.5 py-1 rounded-lg font-medium">
            PVP General: {inlinePrice.toFixed(2)}€
          </span>
...
export function ProductDetailClient({ product, config, catLabel }: Props) {
  ...
  const [inlineDesc, setInlineDesc] = useState(product.description || product.shortDescription || "")
      ...
      <div className="grid md:grid-cols-2 gap-10 mb-16">
        ...
        <div className="flex flex-col gap-5">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight">
            <InlineEdit value={inlineTitle} onSave={setInlineTitle} className="w-full">
              {inlineTitle}
            </InlineEdit>
          </h1>
          ...
          {/* 3. Price */}
          <PriceDisplay product={product} color={color} inlinePrice={inlinePrice} onPriceChange={(v) => setInlinePrice(parseFloat(v) || inlinePrice)} />

          {/* 4. Stock */}
          <div className="flex items-center gap-3">
            <div className={`inline-flex items-center gap-2 text-sm font-semibold ${inlineStock ? "text-green-600" : "text-red-500"}`}>
              <div className={`w-2 h-2 rounded-full ${inlineStock ? "bg-green-500" : "bg-red-400"}`} />
            ...
            {isAdmin && (
              <button
                ...
                style={{ borderColor: inlineStock ? "#16a34a40" : "#dc262640", color: inlineStock ? "#16a34a" : "#dc2626" }}
              >
          ...
          <div className="text-sm text-gray-600 leading-relaxed border-l-4 pl-4" style={{ borderColor: `${color}60` }}>
            <InlineEdit type="textarea" value={inlineDesc} onSave={setInlineDesc} className="w-full">
              {inlineDesc}
            </InlineEdit>
          </div>
