...
export interface LocalProduct {
  ...
  name: string;
  slug: string;
  price: number; // PVP Público (precio público, visible a todos)
  ...
  costPrice?: number; // Precio de coste — SOLO visible para admin, nunca al público
...
> = {
  ...
  panini: {
    ...
    color: "#16a34a",
    bgColor: "#dcfce7",
    description: "Cromos y colecciones oficiales de fútbol, NBA, Star Wars y mucho más.",
    emoji: "⚽",
  },
  digimon: {
    name: "Digimon TCG",
    color: "#2563eb",
    bgColor: "#dbeafe",
    description: "Booster Boxes, Starter Decks y singles del juego de cartas Digimon.",
    emoji: "🦖",
  },
};
...
export const PRODUCTS: LocalProduct[] = [
  ...
  },
  {
    ...
    category: "singles",
    game: "magic",
    images: ["https://cards.scryfall.io/large/front/a/9/a9738cda-adb1-47fb-9f4c-ecd930228c4d.jpg"],
    inStock: true,
    isNew: false,
    isFeatured: true,
    language: "EN",
...
export function getNewProducts(limit = 8): LocalProduct[] {
  return PRODUCTS.filter((p) => p.isNew && p.inStock && isNotCard(p)).slice(0, limit);
}
