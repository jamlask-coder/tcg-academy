...
function NumCell({
  ...
  if (editing) {
    return (
      <input
        ...
        className={`w-full rounded border-2 border-[#1a3a5c] px-1.5 py-0.5 text-right focus:outline-none ${small ? "text-[11px]" : "text-xs"} font-mono`}
      />
    );
  }
  ...
  return (
    <button
      ...
      className={`w-full cursor-text rounded px-1.5 py-0.5 text-right font-mono transition hover:bg-blue-50 ${small ? "text-[11px]" : "text-xs"} ${dirty ? "font-bold text-[#1a3a5c]" : "text-gray-700"}`}
    >
      {value.toFixed(2)}€
    </button>
  );
...
function DateCell({
  ...
  return (
    <input
      ...
      className={`w-full rounded border px-1 py-0.5 text-[11px] focus:border-[#1a3a5c] focus:outline-none ${
        expired
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex justify-end gap-3 border-t border-gray-100 px-6 py-4">
          ...
          <button
            onClick={onConfirm}
            className={`rounded-lg px-4 py-2 text-sm font-semibold text-white transition ${danger ? "bg-red-500 hover:bg-red-600" : "bg-[#1a3a5c] hover:bg-[#2d6a9f]"}`}
          >
            {confirmLabel}
          </button>
        </div>
...
export default function PreciosPage() {
  ...
  return (
    <div className="flex min-h-0 flex-col">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-gray-200 bg-white p-4">
        <div className="relative min-w-[180px] flex-1">
          ...
          <input
            ...
            maxLength={100}
            className="w-full rounded-lg border border-gray-200 py-2 pr-3 pl-8 text-xs focus:border-[#1a3a5c] focus:outline-none"
          />
        </div>
        <select
          value={filterGame}
          ...
          className="rounded-lg border border-gray-200 px-2 py-2 text-xs focus:border-[#1a3a5c] focus:outline-none"
        >
      ...
      {/* ── Bulk toolbar ── */}
      {selected.size > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-3 rounded-xl bg-[#1a3a5c] px-4 py-2.5 text-xs text-white">
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-2 py-2.5 text-right">
                  <button
                    className="ml-auto flex items-center gap-1 font-semibold text-gray-600 hover:text-[#1a3a5c]"
              ...
              {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
                ...
                return (
                  <button
                    key={pg}
                    onClick={() => setPage(pg)}
                    className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs transition ${pg === page ? "bg-[#1a3a5c] text-white" : "border border-gray-200 hover:bg-white"}`}
                  >
                    {pg}
                  </button>
                );
        ...
        <div className="sticky bottom-0 z-20 -mx-4 flex flex-wrap items-center gap-3 rounded-t-2xl border-t border-gray-200 bg-white px-4 py-3 shadow-lg sm:-mx-6">
          ...
          <div className="ml-auto flex gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-[#1a3a5c] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#2d6a9f]"
            >
          ...
          <ul className="max-h-40 space-y-1 overflow-y-auto rounded-xl bg-gray-50 p-3 text-xs">
            {dirtySummary.map((r) => (
              ...
              >
                ...
                <span className="flex-shrink-0 font-mono font-bold text-[#1a3a5c]">
                  {r.price.toFixed(2)}€
                </span>
              </li>
            ))}
          ...
          {bulkAction !== "remove" && (
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="0"
                className="w-24 rounded-lg border-2 border-[#1a3a5c] px-3 py-2 text-center text-sm font-bold focus:outline-none"
              />
    ...
    return (
      <span className="inline-flex items-center gap-0.5">
        <input
          ...
          className="w-10 rounded border-2 border-[#1a3a5c] bg-white px-1 py-0.5 text-center text-[11px] font-bold focus:outline-none"
        />
