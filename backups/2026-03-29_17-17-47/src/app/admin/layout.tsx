...
import { useEffect, useState, useState as useLocalState } from "react";
...
function useSidebarBadges() {
  const [newOrders, setNewOrders] = useLocalState(0);
  const [unreadMsgs, setUnreadMsgs] = useLocalState(0);
...
function SidebarContent({
  ...
  const { newOrders, unreadMsgs } = useSidebarBadges();

  return (
    <div className="flex h-full flex-col">
      {/* Admin badge */}
      <div className="mb-4 flex-shrink-0 rounded-2xl bg-[#1a3a5c] p-4 text-white">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-amber-400 px-2 py-0.5 text-[10px] font-black text-[#1a3a5c] uppercase">
            Admin
          </span>
        </div>
      </div>
...
function Breadcrumb({ pathname }: { pathname: string }) {
  ...
  return (
    <nav className="mb-4 flex items-center gap-1.5 text-sm text-gray-500">
      <Link href="/admin" className="transition hover:text-[#1a3a5c]">
        Admin
      </Link>
    ...
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
      ...
      <div className="mb-4 flex items-center gap-3 lg:hidden">
        <button
          ...
          className="flex min-h-[44px] items-center gap-2 rounded-xl bg-[#1a3a5c] px-4 py-2.5 text-sm font-semibold text-white"
        >
