    ...
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl">
      ...
      >
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          ...
          {product.featured && (
            <span className="rounded-full bg-[#1a3a5c] px-2 py-0.5 text-[10px] font-bold text-white">
              NUEVO
            </span>
          )}
          {!inStock && (
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        ...
        <Link href={`/producto/${product.slug}`}>
          <h3 className="line-clamp-2 text-sm leading-tight font-semibold text-gray-800 transition hover:text-[#1a3a5c]">
            {product.name}
          </h3>
        </Link>
